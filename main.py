...
async def set_delay(message: types.Message):
    global RATE_DELAY
    try:
        new_delay = float(message.text.split()[1])
        if new_delay < 0.5:
            new_delay = 0.5
        RATE_DELAY = new_delay
        await message.answer(f"✅ Rate delay set to **{RATE_DELAY}** seconds")
    except:
        await message.answer("Usage: /delay 2.0\n\nCurrent delay: " + str(RATE_DELAY))
...
async def main():
    global checker
    checker = CrunchyrollChecker(proxies)
    print("✅ Bot started | Rate delay:", RATE_DELAY)
